...
- **4i ✅** Exportação contábil: `exportFiscalBook` gera o livro de saídas em CSV (data, número, chave, situação, cliente, CFOP, valor, forma de pagamento) e o XML de arquivamento de cada documento emitido/cancelado, com filtro por período e unidade; download em `/fiscal` (`FiscalExportCard`). Agente conectado à ferramenta.
- **4i ✅** Exportação contábil: `exportFiscalBook` gera o livro de saídas em CSV e o XML de arquivamento de cada documento emitido/cancelado, com filtro por período e unidade; download em `/fiscal` (`FiscalExportCard`). Agente conectado à ferramenta.
...
- **4n ✅** Consulta de documento fiscal: `consultFiscalDocument ({ sale_id | chave | numero })` retorna modelo (65/55), situação, número, chave, destinatário, valor, prazo de cancelamento (`pode_cancelar`), eventos (`FiscalEvent`) e devoluções (`SaleReturn`) do documento. Agente conectado à ferramenta.
